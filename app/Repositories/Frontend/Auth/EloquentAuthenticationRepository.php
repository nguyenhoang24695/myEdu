<?php namespace App\Repositories\Frontend\Auth;

use App\Models\User;
use App\Exceptions\GeneralException;
use App\Repositories\Frontend\User\UserContract;
use Exception;
use GuzzleHttp\Client;
use Illuminate\Contracts\Auth\Guard;
use Illuminate\Support\Facades\Lang;
use Laravel\Socialite\Contracts\Factory as Socialite;
use App\Events\Frontend\Auth\UserLoggedIn;
use App\Events\Frontend\Auth\UserLoggedOut;

/**
 * Class Registrar
 * @package App\Services
 */
class EloquentAuthenticationRepository implements AuthenticationContract {

	/**
	 * @var Socialite
	 */
	private $socialite;
	/**
	 * @var Guard
	 */
	private $auth;
	/**
	 * @var UserContract
	 */
	private $users;

	/**
	 * @param Socialite $socialite
	 * @param Guard $auth
	 * @param UserContract $users
	 */
	public function __construct(Socialite $socialite, Guard $auth, UserContract $users) {
		$this->socialite = $socialite;
		$this->users = $users;
		$this->auth = $auth;
	}

    /**
     * Create a new user instance after a valid registration.
     *
     * @param  array $data
     * @param bool $provider
     * @return User
     */
	public function create(array $data, $provider = false)
	{
		return $this->users->create($data, $provider = false);
	}

	/**
	 * @param $request
	 * @return \Illuminate\Http\RedirectResponse
	 * @throws GeneralException
	 */
	public function login($request) {
		if ($this->auth->attempt($request->only('email', 'password'), $request->has('remember')))
		{

			if ($this->auth->user()->status == 0)
			{
				$this->auth->logout();
				throw new GeneralException(trans('auth.deactivated'));
			}

			if ($this->auth->user()->status == 2)
			{
				$this->auth->logout();
				throw new GeneralException(trans('auth.banned'));
			}

			if ($this->auth->user()->confirmed == 0)
			{
				$user_id = $this->auth->user()->id;
				$this->auth->logout();
				throw new GeneralException(trans('auth.confirmed').", Hoặc ".'<a href="'.route('account.confirm.resend', $user_id).'">Nhấp vào đây</a>'." gửi lại email xác nhận.");
			}

			event(new UserLoggedIn($this->auth->user()));
			return true;
		}

		throw new GeneralException(trans('auth.loginfalse'));
	}

	/**
	 * @param $user
	 * @return true
	 */

	public function loginWithIdvg($user) {
		$this->auth->login($user, true);
		event(new UserLoggedIn($this->auth->user()));
		return true;
	}

	public function logoutWithIdvg(){
		$this->auth->logout();
	}

    /**
     * @param $token
     * @return mixed
     */
    public function loginWithMWID($token)
    {
        $headers = [
            'Accept' => 'application/json',
            'Authorization' => 'Bearer ' . $token
        ];

        try {
            $http = new Client;
            $response = $http->get('http://account.mywork.com.vn/api/profile/me', [
                'headers' => $headers
            ]);
            $result = json_decode((string)$response->getBody(), true);

            // if ok -> build session mw
            if ($result && isset($result['data']) && isset($result['data']['id'])) {
                $user_info = $result['data'];
                // check user is existed
                $user = User::where('email', $user_info['email'])->first();
                if (!$user) {
                    $user = $this->create([
                        'name' => $user_info['name'],
                        'email' => $user_info['email'],
                        'password' => md5('mywork.vn'),
                    ], true);
                }
                // login with user
                $this->auth->login($user, true);
                event(new UserLoggedIn($this->auth->user()));
            }
        } catch (Exception $ex) {}
    }

    /**
     * @param $data
     * @return mixed
     */
    public function logoutWithMWID($data)
    {

    }

	/**
	 * Log the user out and fire an event
	 */
	public function logout() {
		event(new UserLoggedOut($this->auth->user()));
		$this->auth->logout();
	}

	/**
	 * Socialite Functions
	 */

	/**
	 * @param $request
	 * @param $provider
	 * @return bool|\Symfony\Component\HttpFoundation\RedirectResponse
	 */
	public function loginThirdParty($request, $provider) {
		if (! $request) return $this->getAuthorizationFirst($provider);
		$user = $this->users->findByUserNameOrCreate($this->getSocialUser($provider), $provider);
		$this->auth->login($user, true);
		event(new UserLoggedIn($user));
		return redirect()->route('frontend.dashboard');
	}

	/**
	 * @param $provider
	 * @return \Symfony\Component\HttpFoundation\RedirectResponse
	 */
	public function getAuthorizationFirst($provider) {
		if ($provider == "google") {
			/*
			 * Only allows google to grab email address
			 * Default scopes array also has: 'https://www.googleapis.com/auth/plus.login'
			 * https://medium.com/@njovin/fixing-laravel-socialite-s-google-permissions-2b0ef8c18205
			 */
			$scopes = [
				'https://www.googleapis.com/auth/plus.me',
				'https://www.googleapis.com/auth/plus.profile.emails.read',
			];
			return $this->socialite->driver($provider)->scopes($scopes)->redirect();
		}
		return $this->socialite->driver($provider)->redirect();
	}

	/**
	 * @param $provider
	 * @return \Laravel\Socialite\Contracts\User
	 */
	public function getSocialUser($provider) {
		return $this->socialite->driver($provider)->user();
	}

	/**
	 * @param $token
	 * @return mixed
	 */
	public function confirmAccount($token) {
		$user = $this->users->confirmAccount($token);
		$this->auth->login($user,true);
		return $user;
	}

	/**
	 * @param $user_id
	 * @return mixed
	 */
	public function resendConfirmationEmail($user_id) {
		return $this->users->sendConfirmationEmail($user_id);
	}
}
