...
class CallRestful
{
    ...
    {
        $username = config('money.gates.bao_kim.pro_api_user');
        $password = config('money.gates.bao_kim.pro_api_pwd');
        $private_key = config('money.gates.bao_kim.pro_private_key');
        $server = config('money.gates.bao_kim.pro_payment_url');
    ...
    private function httpBuildQuery($formData, $numericPrefix = '', $argSeparator = '&', $arrName = '')
    {
        ...
        foreach ($formData as $k => $v) {
            ...
            if (is_array($v)) $query[] = $this->httpBuildQuery($v, $numericPrefix, $argSeparator, $k);
