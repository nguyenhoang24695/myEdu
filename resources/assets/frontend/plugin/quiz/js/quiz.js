...
(function($){
    $.slickQuiz = function(element, options) {
        var plugin   = this,
            ...
            defaults = {
                checkAnswerText:  'Kiểm tra',
                nextQuestionText: 'Tiếp tục &raquo;',
                backButtonText: '',
                completeQuizText: 'Hoàn thành',
                tryAgainText: 'Làm lại',
                questionCountText: 'Câu hỏi %current trong %total',
                preventUnansweredText: 'Bạn phải chọn ít nhất một câu trả lời.',
                questionTemplateText:  '%count. %text',
                scoreTemplateText: '%score / %total',
                nameTemplateText:  '<span>Bài kiểm tra: </span>%name',
        ...
        plugin.method = {
            ...
            setupQuiz: function(options) { // use 'options' object to pass args
                ...
                if (plugin.config.tryAgainText && plugin.config.tryAgainText !== '') {
                    $quizResultsCopy.append('<p class="button_tryAgain"><a class="button btn btn-info button_again ' + tryAgainClass + '" href="#">' + plugin.config.tryAgainText + '</a></p>');
                }
                ...
                for (i in questions) {
                    if (questions.hasOwnProperty(i)) {
                        ...
                        if (plugin.config.backButtonText && plugin.config.backButtonText !== '') {
                            questionHTML.append('<a href="#" class="button btn btn-primary ' + backToQuestionClass + '">' + plugin.config.backButtonText + '</a>');
                        }
                        ...
                        if (!plugin.config.perQuestionResponseMessaging) {
                            questionHTML.append('<a href="#" class="button btn btn-primary ' + nextQuestionClass + ' ' + checkAnswerClass + '">' + nextText + '</a>');
                        } else {
                            questionHTML.append('<a href="#" class="button btn btn-primary ' + nextQuestionClass + '">' + nextText + '</a>');
                            questionHTML.append('<a href="#" class="button btn btn-primary ' + checkAnswerClass + '">' + plugin.config.checkAnswerText + '</a>');
                        }
            ...
            checkAnswer: function(checkButton, options) {
                ...
                for (i in answers) {
                    if (answers.hasOwnProperty(i)) {
                        ...
                        if (answer.correct) {
                            ...
                            html_note += '<p class="text-success">✔ '+answer.option+'</p><p class="text-note"><i class="fa fa-hand-o-right" aria-hidden="true"></i><small> '+answer.note+'</small></p>';
                        }
                    }
                }

                //Show ra lý do đúng
                $(_responses).find(_correctResponse).html(questions[questionIndex].correct + html_note);
                ...
                if (plugin.config.preventUnanswered && selectedAnswers.length === 0) {
                    ...
                    $.notify({message: plugin.config.preventUnansweredText}, {type: 'danger'});
                    return false;
                }
            ...
            completeQuiz: function(options) {
                ...
                //Thêm chỗ hiển thị thông báo
                if ($('.button_direc').length > 0) {
                    $quizResultsCopy.find('.button_direc').remove();
                }

                if($('.text_mess').length > 0){
                    $quizResultsCopy.find('.text_mess').remove();
                }

                if(displayScore == questionCount){
                    $text_append = '<p class="text-success text_mess"><i class="fa fa-smile-o"></i> '+quizValues.info.messTrue+'</p>';
                    if(quizValues.info.urlNext != ''){
                        $quizResultsCopy.append('<p><a class="button btn btn-default next_lecture button_direc" href="'+quizValues.info.urlNext+'">Bài giảng tiếp theo</a></p>');
                    }
                } else {
                    $text_append = '<p class="text-danger text_mess"><i class="fa fa-frown-o"></i> '+quizValues.info.messFalse+'</p>';
                    if(quizValues.info.urlBack != ''){
                        $quizResultsCopy.append('<p><a class="button btn btn-default back_lecture button_direc" href="'+quizValues.info.urlBack+'">Quay lại bài giảng</a></p>');
                    }
                }

                if($text_append != ''){
                    $($text_append).insertBefore($quizResultsCopy.find('.button_tryAgain'));
                }
                /************END**************/
