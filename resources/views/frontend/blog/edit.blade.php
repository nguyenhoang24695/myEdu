                ...
                <section>
                  <div class="panel unibee-box" >
                    <div class="panel-heading">
                      <h3 class="panel-title">Sửa Blog</h3>
                    </div>
                  ...
                  <div class="panel create_blog">
                      <div class="panel-body ">
                        <form  enctype="multipart/form-data" method="POST" action="{{ url('/blog/'.$blog->id.'/edit') }}">
        								  <div class="form-group">
                            <label >Chọn danh mục</label>
                            <select class="selectpicker select-control" name="blo_cate">
                              <option value="">Chọn danh mục</option>
                              @foreach ($blog_cate as $key => $value)
                                <option value="{{ $value->id }}" {{ ($blog->blo_cate == $value->id) ? "selected":"" }}>{{ $value->blc_title }}</option>
                              @endforeach
                            </select>
                          ...
                          <div class="form-group">
        								    ...
        								    <input type="text" class="form-control" name="blo_title" placeholder="Tiêu đề blog" value="{{ Request::old('blo_title', $blog->blo_title) }}">
                            ...
                            <textarea name="blo_summary" class="form-control" rows="3" placeholder="Mô tả ngắn">{{ Request::old('blo_summary', $blog->blo_summary) }}</textarea>
                            ...
                            <img style="margin-top: 10px" src="{!! App\Core\MyStorage::get_image_blog_link($blog->blo_disk,$blog->blo_path,'blog_small'); !!}">
        								  </div>
        								  <div class="form-group">
        								    <label >Nội dung</label>
        								    <textarea name="blo_content" class="form-control summernote">{!! Request::old('blo_content', $content) !!}</textarea>
        								  </div>
                          <div class="form-group">
                            <label >Chuyển hướng</label>
                            <div class="radio">
                              <label style="margin-right: 10px">
                                <input type="radio" name="redirect" value="1" checked>
                                Về danh sách
                              </label>
                              <label>
                          ...
                          <div class="form-group">
                            ...
                            <div class="radio">
                              <label style="margin-right: 10px" data-toggle="tooltip" data-placement="top" title="BQT sẽ duyệt bài của bạn và đưa lên trang chủ">
                                <input type="radio" name="public" value="1" {{ ($blog->public == 1) ? "checked":"" }} >
                                Công khai
                              </label>
                              <label style="margin-right: 10px" data-toggle="tooltip" data-placement="top" title="Blog của bạn sẽ chỉ xuất hiện ở trang cá nhân của bạn">
                                <input type="radio" name="public" value="2" {{ ($blog->public == 2) ? "checked":"" }} >
                                Ẩn danh
                              </label>
                              <label data-toggle="tooltip" data-placement="top" title="Sẽ không ai biết đến Blog của bạn">
                                <input type="radio" name="public" value="0" {{ ($blog->public == 0) ? "checked":"" }}>
                                Riêng tư
                              </label>
                            </div>
                          </div>
