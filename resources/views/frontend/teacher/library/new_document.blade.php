...
@section('sub_content')
  <div class="panel wiget_box">
    <div class="panel-heading">
      <h3 class="panel-title">{{trans('course.add_document')}}</h3>
    </div>
    <div class="panel-body ">
      <div class="row">
        {!! Form::open(['name' => 'frm_add_course', 'class' => 'form-horizontal', 'method' => 'post', 'url' => route('teacher.my_library.add_document')]) !!}
        <div class="col-xs-12">
          ...
          <div class="form-group {{ $errors->has('tmp_file') ? 'has-error' : '' }}">
            ...
            <div class="col-md-offset-2" id="">
              <div class="container-fluid">
                ...
                <div id="upload_notice" class="has-error"
                     style="display: {{ $errors->has('tmp_file') ? 'block' : 'none' }}">
                  <label class="control-label">
                    {{trans('validation.upload_notice',
                                ['size' => config('flysystem.max_size.document')/1024/1024 . "MB",
                                 'exts' => implode(',',config('flysystem.exts.document'))])}}
                  </label>
                </div>
              </div>
            </div>
          ...
          <div class="form-group {{ $errors->has('aud_title') ? 'has-error' : '' }}">
            ...
            <div class="col-md-10">
              {!! Form::text('doc_title', null, ['class' => 'form-control', 'placeholder' => trans('common.name')]) !!}
              {!! $errors->first('doc_title', '<label class="control-label" for="inputError">:message</label>') !!}
            </div>
          </div>
          ...
          <div class="form-group {{ $errors->has('doc_description') ? 'has-error' : '' }}">
            ...
            <div class="col-md-10">
              {!! Form::textarea('doc_description', null, ['class' => 'form-control', 'rows' => 5, 'placeholder' => trans('common.description')]) !!}
              {!! $errors->first('doc_description', '<label class="control-label" for="inputError">:message</label>') !!}
            </div>
          </div>
...
@section('after-scripts-end')
  ...
  {!! HTML::script('frontend/js/teacher/add_document.js') !!}
@endsection