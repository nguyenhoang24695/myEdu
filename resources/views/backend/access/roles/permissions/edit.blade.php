        ...
        <div class="form-group">
          ...
          <div class="col-lg-10">
            <input type="text" name="name" class="form-control" placeholder="Permission Name"
                   {{$permission->system == 1 ? 'readonly="readonly"' : ''}} value="{{$permission->name}}"/>
          </div>
        </div>
        <!--form control-->
            ...
            <div class="system-permission-switch">
              <div class="onoffswitch">
                <input type="checkbox" name="system"
                       {{$permission->system == 1 ? 'checked="checked"' : ''}} class="toggleBtn onoffswitch-checkbox"
                       id="system">
            ...
            @if (count($roles) > 0)
              @foreach($roles as $role)
                ...
                <div class="create-permissions-switch">
                  <div class="onoffswitch">
                    <input type="checkbox"
                           {{$role->id == 1 && Config::get('access.roles.administrator_forced') ? "disabled='disabled'" : ''}} {{in_array($role->id, $permission_roles) || ($role->id == 1 && Config::get('access.roles.administrator_forced')) ? 'checked="checked"' : ""}} value="{{$role->id}}"
                           name="permission_roles[]" class="toggleBtn onoffswitch-checkbox" id="role-{{$role->id}}">
